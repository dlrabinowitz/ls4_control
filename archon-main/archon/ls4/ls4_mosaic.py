...
class LS4_Mosaic():
    ...
    def __init__(self,conf=None):
      ...
      image_list = self.conf['images']

      assert image_list is not None, "unspecified image list"
      self.num_images = len(image_list)
      

      self.num_dark_images=0
      if self.conf['dark_images'] is not None:
         self.num_dark_images=len(self.conf['dark_images'])
         assert self.num_dark_images == self.num_images, "dark image mismatch: %d %d" %\
                 (self.num_images,self.num_dark_images)

    def write_fits(self,data=None,header=None,output='mos.fits'):
    ...
    def subtract_col_bias(self,data=None,pre_x=0,post_x=0,pre_y=0,post_y=0,amp_name=None,y1=None,y2=None):
    ...
    def subtract_row_bias(self,data=None,pre_x=0,post_x=0,pre_y=0,post_y=0,amp_name=None):
    ...
    def assemble(self):
      im_list = self.conf['images']
      im_list_dark = self.conf['dark_images']
      ...
      ccd_sec=im_head['CCDSEC']
      ccd_sec = ccd_sec.replace("[","")
      ccd_sec = ccd_sec.replace("]","")
      ccd_sec = ccd_sec.replace(":"," ")
      ccd_sec = ccd_sec.replace(","," ")
      ccd_sec = ccd_sec.split()
      ccd_width = int(ccd_sec[1])-int(ccd_sec[0])+ 1
      ccd_height = int(ccd_sec[3])-int(ccd_sec[2])+ 1
      ccd_width = 1024
      ccd_height = 1024
      prescan_x = 6
      postscan_x = width - prescan_x - ccd_width
      prescan_y = 0
      postscan_y = height - ccd_height
      if postscan_y < 0:
         postscan_y = 0
      ...
      for im in im_list:
         ...
         if self.conf['bias']:
           im_data,bias,rms = self.subtract_col_bias(im_data_raw,prescan_x, postscan_x,prescan_y,postscan_y,amp_name,y1,y2)
         else:
           im_data=im_data_raw
           bias=0.0
           rms=0.0
      ...
      self.write_fits(mos_data,im_head,self.conf['output'])
...
if __name__ == "__main__":
    ...
    def get_params():
      ...
      parser.add_argument('--images', metavar='i', type=namelist, default=None)
      parser.add_argument('--dark_images', metavar='d', type=namelist, default=None)
      parser.add_argument('--output', metavar='c', type=str, default='test.fits')
      parser.add_argument('--bias', metavar='b', type=bool, default=True) 
