...
def assemble_mosaic(conf=None):
  ...
  num_dark_images=0
  amp_selection = conf['amp_selection'].upper()
  assert amp_selection in ['LEFT','RIGHT','BOTH'], 'unrecognized amp selection'

  if amp_selection in ['LEFT','RIGHT']:
     amps_per_ccd = 1
     cols_per_ccd = 2048
  else:
     amps_per_ccd = 2
  ...
  prescan_x = 6
  postscan_x = width - prescan_x - cols_per_ccd
  prescan_y = 0
  postscan_y = height - rows_per_ccd
  if postscan_y < 0:
     postscan_y = 0
  ...
  for im in im_list:
     ...
     #HERE
     print("shape mos_data : %s  im_data: %s  y0: %d height: %d x0: %d width %d" %\
           (mos_data.shape,im_data.shape,y0, height,x0, width))
...
def get_params():
  ...
  parser.add_argument('--amp_selection', metavar='a', type=str, default="BOTH")
